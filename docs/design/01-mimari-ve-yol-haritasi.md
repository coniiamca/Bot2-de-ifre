...
| Faz 2–3 — ilk dilim | **Kod tamam** | Araştırma çekirdeği (`quanta.research`, `research` extra'sı): point-in-time saatlik panel, Tier-0 bar backtest (komisyon, kayma, gerçek funding), CPCV / DSR / PBO / Newey–West / durağan bootstrap, sentetik piyasa öz-testleri, hayatta kalma yanlılığı olmayan aylık evren (tüm USDT perp'ler, kaldırılanlar dahil), ön-kayıt + deneme defteri + tek seferlik kilitli dönem. İlk hipotezler: H6 (trend, ilk-10) ELENDİ; H6B (trend, her ay ilk-5; H6'nın denemeleri de sayılır) ELENDİ; H2 (kaldıraç kalabalığı) SONUÇSUZ; H5 (çeyrek saat açılış dengesizliği) ELENDİ (maliyet öncesi etki var, maliyetin yarısı); dakikalık hızlı işlem I1–I4 (geri dönüş, kırılım, akış dengesizliği, funding saati; havuz 5/10/20 coin) dördü de ELENDİ; M1 (mumlardan öğrenen model, USDC kontratlarında sıfır komisyonlu limit emir) 15 kapının 13'ünü geçti ama DSR ve likidasyon kapısında ELENDİ — `docs/research/sonuclar/`, `docs/research/03-hizli-islem.md`, `docs/research/04-model-m1.md`. |
| Faz 4 — dilim 1 (demo) | **Kod tamam, anahtar bekliyor** | Binance demo hesabında emir altyapısı: Ed25519/HMAC imza, imzalı REST (emir, algo zarar kes, `countdownCancelAll`, hesap/pozisyon), kullanıcı akışı, OMS (SQLite, önce niyet kaydı, BİLİNMİYOR→sorgu, uzlaştırma — ADR-012), risk (§13.4 limitleri, kill switch seviyeleri, günlük zarar, bayat veri, fiyat bandı), acil kapatma, kanarya test çevrimleri ve günlük tatbikatlar, durum sayfasında "Demo işlem". Sahte borsaya karşı uçtan uca testli. Sunucuda `sudo quanta-demo-anahtar` sonrası çalışır. Kalan: ayrı guardian, WS API ile emir, kaos testleri, 7 günlük soak. |
